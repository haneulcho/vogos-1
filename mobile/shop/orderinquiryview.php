...
<section class="sod_fin_list">
	...
	</section> <!-- 주문상품 정보 section END -->

	<div class="inquiry_price origin">
		<div class="label"><span>(배송비포함)</span></div>
		<div class="price"></div>
	</div>
	<div class="inquiry_price discount">
		<div class="label"><span>(포인트사용)</span></div>
		<div class="price"></div>
	</div>
	<div class="inquiry_price total rd">
		<div class="label"><span>(주문금액 - 할인금액)</span></div>
		<div class="price">=</div>
	</div>
	<!-- 주문금액, 할인금액, 결제금액 END -->
...
<!-- 주문한 사람, 받는 사람 -->
<div class="inquiry_person">
	<h2>주문한 사람</h2>
	<dl class="mp_lst_txt">
		<dt><i class="ion-android-person"></i></dt>
		<dd><?php echo get_text($od['od_name']); ?></dd>
		<dt><i class="ion-email"></i></dt>
		<dd><?php echo get_text($od['od_email']); ?></dd>
		<dt><i class="ion-android-call"></i></dt>
		<dd>
			<?php if($od['od_tel']) { ?>
			<?php echo ($od['od_tel'] ? get_text($od['od_tel']).' / ' : '미등록 / '); ?>
			<?php } ?>
			<?php if($od['od_hp']) { ?>
			<?php echo ($od['od_hp'] ? get_text($od['od_hp']) : '미등록'); ?>
			<?php } ?>
		</dd>
		<dt><i class="ion-android-home"></i></dt>
	...
	<dl class="mp_lst_txt">
		...
		<dd>
			<?php if($od['od_b_tel']) { ?>
			<?php echo ($od['od_b_tel'] ? get_text($od['od_b_tel']).' / ' : '미등록 / '); ?>
			<?php } ?>
			<?php if($od['od_b_hp']) { ?>
			<?php echo ($od['od_b_hp'] ? get_text($od['od_b_hp']) : '미등록'); ?>
			<?php } ?>
		</dd>
		<dt><i class="ion-android-home"></i></dt>
		...
		<?php if($default['de_hope_date_use']) { // 희망배송일을 사용할 경우 표시 ?>
			<dt><i class=""></i></dt>
			<dd><?php echo substr($od['od_hope_date'],0,10).' ('.get_yoil($od['od_hope_date']).')' ;?></dd>
		<?php } ?>

		<?php if($od['od_memo']) { // 전하실 말씀을 적었을 경우 표시 ?>
			<dt><i class="ion-chatboxes"></i></dt>
			<dd><?php echo conv_content($od['od_memo'], 0); ?></dd>
		<?php } ?>
	</dl>
</div>
<!-- 주문한 사람, 받는 사람 (inquiry_person) END -->
